...
<%
	...
	String passInfo = "审核通过，进入生产环境测试";
	String notPassInfo = "审核驳回，重新进行上线前环境测试";
...
function chooseLinkInfo(){
	if(orderResult == 'pass'){
		...
		next_linkId ='<%=WorkflowParam.getWorkflow("eaeSubTaskTestBTVerify").getLinkId()%>';
		next_linkNo = '<%=WorkflowParam.getWorkflow("eaeSubTaskTestBTVerify").getLinkNo()%>';
		next_roleCode = '<%=WorkflowParam.getWorkflow("eaeSubTaskTestBTVerify").getRoleCode()%>';
		stdMark='1';
	}else if(orderResult == 'notPass'){
		next_linkId ='<%=WorkflowParam.getWorkflow("eaeSubTaskTestQuaRel").getLinkId()%>';
		next_linkNo = '<%=WorkflowParam.getWorkflow("eaeSubTaskTestQuaRel").getLinkNo()%>';
		next_roleCode = '<%=WorkflowParam.getWorkflow("eaeSubTaskTestQuaRel").getRoleCode()%>';
		stdMark='1';
	}
