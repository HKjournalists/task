	...
	<body style="padding:10px 0px 60px 10px">
		<div>
			<jsp:include page="/aiga/workflow/testPlan/TestSolidTaskForm.jsp">
				...
				<jsp:param name="taskTypeName" value='<%=java.net.URLEncoder.encode(java.net.URLEncoder.encode("手工回归测试","utf-8"))%>'/>
...
var current_linkNo = '<%=WorkflowParam.getWorkflow("hwRegrTestAllot").getLinkNo()%>';
//下一环节link信息
var	next_linkId ='';
var next_linkNo = '';
var	next_roleCode = '';
...
function chooseLinkInfo(){
	if(orderResult == 'pass'){
		...
		next_linkId ='<%=WorkflowParam.getWorkflow("hwRegrTestAllot").getLinkId()%>';
		next_linkNo = '<%=WorkflowParam.getWorkflow("hwRegrTestAllot").getLinkNo()%>';
		next_roleCode = '<%=WorkflowParam.getWorkflow("hwRegrTestAllot").getRoleCode()%>';
	}
