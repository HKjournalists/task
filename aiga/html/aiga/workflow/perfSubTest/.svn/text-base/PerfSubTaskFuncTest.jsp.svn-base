...
function chooseLinkInfo(){
	if(orderResult == 'pass'){
		...
		next_linkId ='<%=WorkflowParam.getWorkflow("perfSubTaskFuncTest").getLinkId()%>';
		next_linkNo = '<%=WorkflowParam.getWorkflow("perfSubTaskFuncTest").getLinkNo()%>';
		next_roleCode = '<%=WorkflowParam.getWorkflow("perfSubTaskFuncTest").getRoleCode()%>';
		stdMark='1';
	}
