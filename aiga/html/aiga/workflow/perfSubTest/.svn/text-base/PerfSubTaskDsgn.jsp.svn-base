...
<%
	...
	String passInfo = "性能子任务设计完成，进入准发布测试";
...
//下一环节link信息
var current_linkNo = '<%=WorkflowParam.getWorkflow("perfSubTaskDsgn").getLinkNo()%>';
//下一环节link信息
var	next_linkId ='';
var next_linkNo = '';
var	next_roleCode = '';
...
function chooseLinkInfo(){
	if(orderResult == 'pass'){
		next_linkId ='<%=WorkflowParam.getWorkflow("perfSubTaskQuaRel").getLinkId()%>';
		next_linkNo = '<%=WorkflowParam.getWorkflow("perfSubTaskQuaRel").getLinkNo()%>';
		next_roleCode = '<%=WorkflowParam.getWorkflow("perfSubTaskQuaRel").getRoleCode()%>';
		stdMark='3';
	}else if(orderResult == 'regn'){
		next_linkId ='<%=WorkflowParam.getWorkflow("perfSubTaskDsgn").getLinkId()%>';
		next_linkNo = '<%=WorkflowParam.getWorkflow("perfSubTaskDsgn").getLinkNo()%>';
		next_roleCode = '<%=WorkflowParam.getWorkflow("perfSubTaskDsgn").getRoleCode()%>';
		stdMark='1';
	}
