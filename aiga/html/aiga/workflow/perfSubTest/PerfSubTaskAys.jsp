...
<%
	...
	String passInfo = "性能子任务分析完成，进入测试设计";
...
function chooseLinkInfo(){
	if(orderResult == 'pass'){
		...
		next_linkId ='<%=WorkflowParam.getWorkflow("perfSubTaskAys").getLinkId()%>';
		next_linkNo = '<%=WorkflowParam.getWorkflow("perfSubTaskAys").getLinkNo()%>';
		next_roleCode = '<%=WorkflowParam.getWorkflow("perfSubTaskAys").getRoleCode()%>';
		stdMark='1';
	}
