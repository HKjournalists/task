...
<%
	...
	String passInfo = "代码扫描测试评审完成，代码扫描完成";
	String regnInfo = "转派其他相关人员进行此环节操作";
	String notPassInfo = "代码扫描测试评审不通过，发送测试人员重新执行";
	//工单Id
	...
	<body style="">
		<div>
			<jsp:include page="/aiga/workflow/testPlan/TestSolidTaskForm.jsp">
				...
				<jsp:param name="taskTypeName" value="代码扫描"/>
...
var current_linkNo = '<%=WorkflowParam.getWorkflow("codeScanVerify").getLinkNo()%>';
//下一环节link信息
var	next_linkId ='';
var next_linkNo = '';
var	next_roleCode = '';
...
function chooseLinkInfo(){
	if(orderResult == 'pass'){
		...
		next_linkId ='<%=WorkflowParam.getWorkflow("codeScanVerify").getLinkId()%>';
		next_linkNo = '<%=WorkflowParam.getWorkflow("codeScanVerify").getLinkNo()%>';
		next_roleCode = '<%=WorkflowParam.getWorkflow("codeScanVerify").getRoleCode()%>';
		stdMark='1';
	}
