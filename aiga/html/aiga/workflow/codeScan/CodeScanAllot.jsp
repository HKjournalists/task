	...
	<body style="">
		<div>
			<jsp:include page="/aiga/workflow/testPlan/TestSolidTaskForm.jsp">
				...
				<jsp:param name="taskTypeName" value='java.net.URLEncoder.encode(java.net.URLEncoder.encode("代码扫描","utf-8"))'/>
				<jsp:param name="templateId" value="23000"/>
...
var current_linkNo = '<%=WorkflowParam.getWorkflow("codeScanAllot").getLinkNo()%>';
//下一环节link信息
var	next_linkId ='';
var next_linkNo = '';
var	next_roleCode = '';
...
function chooseLinkInfo(){
	if(orderResult == 'pass'){
		next_linkId ='<%=WorkflowParam.getWorkflow("codeScanRunning").getLinkId()%>';
		next_linkNo = '<%=WorkflowParam.getWorkflow("codeScanRunning").getLinkNo()%>';
		next_roleCode = '<%=WorkflowParam.getWorkflow("codeScanRunning").getRoleCode()%>';
	}else if(orderResult == 'regn'){
		next_linkId ='<%=WorkflowParam.getWorkflow("codeScanAllot").getLinkId()%>';
		next_linkNo = '<%=WorkflowParam.getWorkflow("codeScanAllot").getLinkNo()%>';
		next_roleCode = '<%=WorkflowParam.getWorkflow("codeScanAllot").getRoleCode()%>';
	}
	next_templateId = '23000';
}
