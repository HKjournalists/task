...
<%
	...
	String passInfo = "性能测试接口人审核完成，进入性能测试评审";
	String regnInfo = "转派其他相关人员进行此环节操作";
	String notPassInfo = "性能测试接口人审核不通过，发送测试人员重新执行";
	//工单Id
	...
	<body style="padding:10px 0px 60px 10px">
		<div>
			<jsp:include page="/aiga/workflow/testPlan/TestSolidTaskForm.jsp">
				...
				<jsp:param name="taskTypeName" value="性能测试"/>
...
var current_linkNo = '<%=WorkflowParam.getWorkflow("perfInterfaceVerify").getLinkNo()%>';
//下一环节link信息
var	next_linkId ='';
var next_linkNo = '';
var	next_roleCode = '';
...
function chooseLinkInfo(){
	if(orderResult == 'pass'){
		...
		next_linkId ='<%=WorkflowParam.getWorkflow("perfInterfaceVerify").getLinkId()%>';
		next_linkNo = '<%=WorkflowParam.getWorkflow("perfInterfaceVerify").getLinkNo()%>';
		next_roleCode = '<%=WorkflowParam.getWorkflow("perfInterfaceVerify").getRoleCode()%>';
		stdMark='1';
