...
<html>
	...
	<script type="text/javascript">
		...
		function regeditData() {
			...
			Ext.define('FuncPoint',{
				...
				fields: [
			        ...
			        {name: 'subTaskId',  type: 'int', convert: null, defaultValue: '<%=subTaskId%>'},
			...
			var workflowParamStore = new Ext.data.Store({
	            ...
	            proxy: {
	                type: 'ajax',
	                url: '<%=request.getContextPath()%>/getWorkflowParam.do?query=40000',
	                reader: {
	                    type: 'json',
	                    root: 'data'
	                }
			...
			var searchStore = Ext.create('Ext.data.Store', {
			    ...
			    proxy: {
			        type: 'ajax',
		        	url : '<%=request.getContextPath()%>/searchFunFolder.do?subTaskId=<%=subTaskId%>',
			        reader: {
						type:"json",
						root: 'data'
			        }
		...
		function addFunpTabs() {
			...
			var tabs = new Ext.TabPanel({
				...
				items: [
					new Ext.form.Panel({
						...
						items:[
							{xtype:'textareafield',fieldLabel:'【测试依据/主要需求描述】',name:'testReply',height: 80, width: '90%'<%if(isRead){%>,readOnly: true<%}%>},
							{xtype:'textareafield',fieldLabel:'【需求分析】',name:'reqAnalysis',height: 80, width: '90%'<%if(isRead){%>,readOnly: true<%}%>},
							{xtype:'textareafield',fieldLabel:'【现状测试】',name:'nowTestInfo',height: 80, width: '90%'<%if(isRead){%>,readOnly: true<%}%>},
							{xtype:'combo',name:'usable',fieldLabel:'是否完成',valueField:'value',displayField:'show',store:'usableStore',width: 280<%if(isRead){%>,readOnly: true<%}%>}
						]
					}),
							...
							Ext.create('Ext.grid.Panel', {
							    ...
							    columns: [
							        ...
							        {hidden: true, dataIndex: 'elemId'},
							       	{header: '要素名称', dataIndex: 'elemName',width: 100},
					...
					Ext.create('Ext.grid.Panel', {
						...
						listeners: {
							show: function(curTab, eOpts) {
								Ext.getCmp('hisCaseGrid').getStore().reload();
							}
						}
					}),
					Ext.create('Ext.grid.Panel', {
								...
								items:[
									{
										...
										items:[
											{xtype:'combo',fieldLabel:'测试环境',name:'reasonEnvType',displayField:'show',valueField:'value', store:'reasonEnvType',readOnly: true},
											{xtype:'combo',fieldLabel:'测试类型',name:'reasonPossible',displayField:'show',valueField:'value', store:'possible'},
											{xtype:'combo',fieldLabel:'不可测原因',name:'reasonType',displayField:'show',valueField:'value', store:'reasonType'}
										]
									},
									{xtype:'textareafield',fieldLabel:'备注',name:'reasonDesc', width: 700, height: 60},
									{xtype:'hidden',name:'reasonId'},
									{xtype:'hidden',name:'subTaskId',value:'<%=subTaskId%>'}
								]
							}),
							Ext.create('Ext.form.Panel', {
								id: 'reasonPanel2',
			...
			zTabs.insertHtml("afterBegin",'<div id="addFunBtn" style="position: absolute; right: -16px; top: 5px; z-index: 2;display: none;"><a href="javascript:void(0);" class="W_btn_c fn-mr-20" onclick="addFunArea();Ext.getCmp(\'funTab\').doLayout();return false;"><span>新增功能点</span></a></div>');
			new Ext.form.Panel({
                   	title: '对账测试',
                   	id: 'accountTest',
                   	layout: 'vbox',
                   ...
                   new Ext.form.Panel({
					...
					items:[
						{xtype:'textareafield',fieldLabel:'【数据稽核准确率】',name:'cutOverDataRate',height: 80<%if(isRead){%>,readOnly: true<%}%>},
						{xtype:'textareafield',fieldLabel:'【脚本功能测试通过率】',name:'cutOverFunRate',height: 80<%if(isRead){%>,readOnly: true<%}%>},
						{xtype:'textareafield',fieldLabel:'【脚本性能测试通过率】',name:'cutOverPerRate',height: 80<%if(isRead){%>,readOnly: true<%}%>}
					]
                   });
		...
		function showJoinSysHtml() {
			...
			$.ajax({
				...
				success: function(json) {
					...
					var other = Ext.getCmp("subTaskForm").getForm().findField("joinDebugValsOther").getValue();
					sys1Html += "<span class='otherJoinSpan'><label>其他<input type='text' name='joinDebugValsOther' value='"+other+"' /></label></span>";
					sys2Html += "<span class='otherJoinSpan'><label>其他<input type='text' name='joinDebugValsOther' value='"+other+"' /></label></span>";
					...
					<%if(isRead){%>
					$("#joinSys1 input").attr("disabled", true);
					$("#joinSys2 input").attr("disabled", true);
					<%}%>
				}
			});
		}
		...
		function funSub() {
			...
			*/
			...
			$("#tab1-body").find(".tab1-fun").each(function(index){
				...
				var id = $(this).attr("id");
				var funPNo = id.replace("funP","");
				if(!subCurFun(funPNo, false, false)) {
					correct = false;
					return false;
				}
				/*
		...
		function saveReasons() {
			...
			var res1Data = Ext.getCmp('reasonPanel1').getForm().getValues();
			var res2Data = Ext.getCmp('reasonPanel2').getForm().getValues();
			var res3Data = Ext.getCmp('reasonPanel3').getForm().getValues();
			var reasons = new Array();
			reasons.push(Ext.encode(res1Data));
			reasons.push(Ext.encode(res2Data));
			reasons.push(Ext.encode(res3Data));
			reasons = "[" + reasons + "]";
			$.ajax({
	    		async: false,
				type: 'POST',
				...
				items: [
					{
						...
						border: 0,
						items: [
							{fieldLabel: '功能点名称',labelAlign: 'right',width:350,xtype: 'combo',name: 'sysName',store: 'searchStore',<%if(isRead){%>readOnly: true,<%}%>
            						...
            						select: function(comp, The, eOpts){
					            		...
					            		funForm.getForm().load({
					            			...
					            			success: function(form, action) {
					            				...
					            				form.findField('sysName').setValue(The[0].raw.sysName);
		...
		function copyFunPoint(formId) {
			...
			Ext.getCmp(formId).getForm().submit({
				...
				success: function(form, action) {
			MaskOver();
					addFunArea(action.result.data, true, false);
					Ext.getCmp('funTab').doLayout();
				}
			});
		}
			...
			$.ajax({
				...
				success: function(json) {
					...
					for(var i = 0; i < audits.length; i++) {
						...
						<%if(isRead){%>
						$("#dataAudit input").attr("disabled", true);
						<%}%>
					}
			...
			$.ajax({
				...
				success: function(json) {
					...
					for(var i = 0; i < accounts.length; i++) {
						...
						<%if(isRead){%>
						$("#accountTestTbl input").attr("disabled", true);
						<%}%>
					}
		...
		function manageCase(num) {
			...
			if(funId == null || funId == "" || funId == "0") {
				Ext.Msg.alert("提示","请在功能点名称中输入选择功能点");
				return;
			}
		...
		function relaCase(num) {
			...
			var subTaskTag = Ext.getCmp('subTaskForm').getForm().findField('subTaskTag').getValue();
			if(funId == null || funId == "" || funId == "0") {
				Ext.Msg.alert("提示","请在功能点中输入查询并选择功能点");
				return;
			}
			...
			var win = new top.Ext.window.Window({
				...
				modal: true,
				constrain:true,
				html: "<iframe src='<%=request.getContextPath()%>/aiga/userCase/subTaskCaseRela.jsp?funId="+funId+"&subTaskTag="+subTaskTag+"&relaId="+relaId+"&caseIds="+caseIds+"' frameborder='0' width='100%' height='530'></iframe>"
			});
			win.show();
		}
			...
			var win = new Ext.window.Window({
				...
				items: [
					Ext.create('Ext.grid.Panel',{
						...
						columns: [
							{xtype:'rownumberer'},
					        {header: '要素值', dataIndex: 'elemValue', width:100},
					        {header: '取数方法', dataIndex: 'takeValueMethod',width:200},
					        {header: '备注', dataIndex: 'valueRemark' ,width:200},
					        {header: '修改类型', dataIndex: 'operatorType',
					        	renderer: function(value) {
					        		if(value == "add") {
					        			return "增加";
		...
		function checkBeforeSubmit() {
			...
			var reasonPanel1 = Ext.getCmp('reasonPanel1').getForm();
			var reasonPanel2 = Ext.getCmp('reasonPanel2').getForm();
			var reasonPanel3 = Ext.getCmp('reasonPanel3').getForm();
			/*
			...
			*/
			var pos1 = reasonPanel1.findField("reasonPossible").getValue();
			var pos2 = reasonPanel2.findField("reasonPossible").getValue();
			var pos3 = reasonPanel3.findField("reasonPossible").getValue();
			if(pos1 == null || pos1 == "") {
				Ext.Msg.alert("校验提示", "请填写测试环境-测试类型字段");
				return false;
			}
			if(pos2 == null || pos2 == "") {
				Ext.Msg.alert("校验提示", "请填写准发布环境-测试类型字段");
				return false;
			}
			if(pos3 == null || pos3 == "") {
				Ext.Msg.alert("校验提示", "请填写生产环境-测试类型字段");
				return false;
			}
			var type1 = reasonPanel1.findField("reasonType").getValue();
			var type2 = reasonPanel2.findField("reasonType").getValue();
			var type3 = reasonPanel3.findField("reasonType").getValue();
			if(pos1 != "1" && (type1 == null || type1 == "")) {
				Ext.Msg.alert("校验提示", "请填写测试环境-不可测原因字段");
				return false;
			}
			if(pos2 != "1" && (type2 == null || type2 == "")) {
				Ext.Msg.alert("校验提示", "请填写准发布环境-不可测原因字段");
				return false;
			}
			if(pos3 != "1" && (type3 == null || type3 == "")) {
				Ext.Msg.alert("校验提示", "请填写生产环境-不可测原因字段");
				return false;
			}
		...
		function refreshReasons() {
			Ext.getCmp('reasonPanel1').getForm().reset();
			Ext.getCmp('reasonPanel2').getForm().reset();
			Ext.getCmp('reasonPanel3').getForm().reset();
			Ext.getCmp('reasonPanel1').getForm().load({
				url: '<%=request.getContextPath()%>/getReasonByTaskEnv.do',
				params:{subTaskId:'<%=subTaskId%>',reasonEnvType:'1'},
	    		async: false,
	    		success: function(form, action) {
	    			form.findField('reasonEnvType').setValue(1);
	    			form.findField('subTaskId').setValue('<%=subTaskId%>');
	    		}
			});
			Ext.getCmp('reasonPanel2').getForm().load({
				url: '<%=request.getContextPath()%>/getReasonByTaskEnv.do',
				params:{subTaskId:'<%=subTaskId%>',reasonEnvType:'2'},
	    		async: false,
	    		success: function(form, action) {
	    			form.findField('reasonEnvType').setValue(2);
	    			form.findField('subTaskId').setValue('<%=subTaskId%>');
	    		}
			});
			Ext.getCmp('reasonPanel3').getForm().load({
				url: '<%=request.getContextPath()%>/getReasonByTaskEnv.do',
				params:{subTaskId:'<%=subTaskId%>',reasonEnvType:'3'},
	    		async: false,
	    		success: function(form, action) {
	    			form.findField('reasonEnvType').setValue(3);
	    			form.findField('subTaskId').setValue('<%=subTaskId%>');
	    		}
			});
		}
