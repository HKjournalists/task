...
<html>
    ...
    <script type="text/javascript">
    	...
    	Ext.onReady(function(){
                ...
                listeners: {
                	load:function(){
            			StaffRoleViewStore.each(function(record){
            				//optionHtml+='<option value="'+record.raw.staffId+'">'+record.raw.staffCode+":"+record.raw.staffName+'</option>';
            			});
                	}
            ...
            var StaffRoleViewComboBox = new Ext.form.ComboBox({
                ...
                listeners: {
                	select :function(combo,records, eOpts ){
                		var currentGridRowModels=Ext.getCmp('subTaskGrid').getSelectionModel().getSelection();
                		if(currentGridRowModels.length==1){
                			currentGridRowModels[0].set('changeStaffId',records[0].raw.staffId);
                		}
                	},
            ...
            var StaffGiveViewStore = new Ext.data.Store({
                id: 'StaffGiveViewStore',
                model: StaffRoleViewModel,
                proxy: {
                    type: 'ajax',
                ...
                listeners:{
                          load : function(store, records, options ){
        	                  StaffGiveViewStore.each(function(record){
            ...
            StaffGiveViewStore.load({params:{roleCode:'WF_TEST_CHD'}});
            var StaffGiveViewComboBox = new Ext.form.ComboBox({
                width: 220,
                store: StaffGiveViewStore,
                name: "creator",
                forceSelection:true,
	            typeAhead:true,
	            minChars:1,
            ...
            var taskStore = Ext.create('Ext.data.Store', {
        	    ...
        	    proxy: {
        	        type: 'ajax',
                	url : '<%=request.getContextPath()%>/getAllTestTask.do',
        	        reader: {
        	            root:"data",
        				type:"json"
        	        }
            ...
            /*var taskComboBox = new Ext.form.ComboBox({
                width: 220,
                store: taskStore,
                fieldLabel: "测试任务",
                valueField: 'taskName',
	       ...
	       var testTaskStatusCombox = new Ext.form.ComboBox({
    		...
    		var subTaskStore = Ext.create('Ext.data.Store', {
			    ...
			    proxy: {
			        type: 'ajax',
		        	url : '<%=request.getContextPath()%>/getTestSubTaskByCon.do?flag=2',
			        reader: {
			            root:"data",
						type:"json",
            			totalProperty:'total'
    		...
    		var subTaskGrid = new Ext.grid.Panel({
        		...
        		**/
    			...
    			columns: [
	        		...
	        		{header: "当前测试人员", width:100,sortable: true, dataIndex: 'testorName'},
	        		//隐藏列
	        		//选择人员下来列表
	        		{ header: '',dataIndex: 'changeStaffId', width: 100 ,hidden: true},
	        		{ header: "选择改派测试人员",width:150,dataIndex: 'changeStaffName', 
    	...
    	function querySubTask() {
			...
			/*var taskId = Ext.getCmp('qryForm').getForm().findField('taskId').getValue();*/
			Ext.StoreMgr.get('subTaskStore').on('beforeload',function(){        // =======翻页时 查询条件     
				Ext.apply(       
					Ext.StoreMgr.get('subTaskStore').proxy.extraParams, {
						...
						beginDate:beginDate,endDate:endDate,isFlag:isFlag
					}      
				);      
			});
			Ext.StoreMgr.get('subTaskStore').loadPage(1);
