...
//下一环节link信息
var current_linkNo = '<%=WorkflowParam.getWorkflow("subTestTaskVerify").getLinkNo()%>';
//下一环节link信息
var	next_linkId ='';
var next_linkNo = '';
var	next_roleCode = '';
...
function chooseLinkInfo(){
	if(orderResult == 'pass'){
		next_linkId ='<%=WorkflowParam.getWorkflow("subTestTaskFuncTest").getLinkId()%>';
		next_linkNo = '<%=WorkflowParam.getWorkflow("subTestTaskFuncTest").getLinkNo()%>';
		next_roleCode = '<%=WorkflowParam.getWorkflow("subTestTaskFuncTest").getRoleCode()%>';
		stdMark='3';
	}else if(orderResult == 'regn'){
		next_linkId ='<%=WorkflowParam.getWorkflow("subTestTaskVerify").getLinkId()%>';
		next_linkNo = '<%=WorkflowParam.getWorkflow("subTestTaskVerify").getLinkNo()%>';
		next_roleCode = '<%=WorkflowParam.getWorkflow("subTestTaskVerify").getRoleCode()%>';
		stdMark='1';
	}else if(orderResult == 'notPass'){
		next_linkId ='<%=WorkflowParam.getWorkflow("subTestTaskDsgn").getLinkId()%>';
		next_linkNo = '<%=WorkflowParam.getWorkflow("subTestTaskDsgn").getLinkNo()%>';
		next_roleCode = '<%=WorkflowParam.getWorkflow("subTestTaskDsgn").getRoleCode()%>';
		stdMark='3';
	}
