...
<%
	...
	String passInfo = "准发布测试通过，进入生产测试";
	String notPassInfo = "测试不通过，同步到缺陷流程";
			...
			<div class="btmFixed">
				...
				<div class="btmFixed_left" id="putongliucheng">
					<ul>
						...
						<li title="<%=notPassInfo %>" onmouseover="this.className='hover'" onmouseout="this.className=''" onclick="sendToDefect()">
...
//下一环节link信息
var current_linkNo = '<%=WorkflowParam.getWorkflow("subTestTaskQuaRel").getLinkNo()%>';
//下一环节link信息
var	next_linkId ='';
var next_linkNo = '';
var	next_roleCode = '';
...
function chooseLinkInfo(){
	if(orderResult == 'pass'){
		next_linkId ='<%=WorkflowParam.getWorkflow("subTestTaskProduce").getLinkId()%>';
		next_linkNo = '<%=WorkflowParam.getWorkflow("subTestTaskProduce").getLinkNo()%>';
		next_roleCode = '<%=WorkflowParam.getWorkflow("subTestTaskProduce").getRoleCode()%>';
		stdMark='3';
	}else if(orderResult == 'regn'){
		next_linkId ='<%=WorkflowParam.getWorkflow("subTestTaskQuaRel").getLinkId()%>';
		next_linkNo = '<%=WorkflowParam.getWorkflow("subTestTaskQuaRel").getLinkNo()%>';
		next_roleCode = '<%=WorkflowParam.getWorkflow("subTestTaskQuaRel").getRoleCode()%>';
		stdMark='1';
	}
	next_templateId = '40000';
}
...
var afterSelect = function(staffs,option){
	...
	//var subTaskClass = Ext.getCmp('subTaskForm').getForm().findField('subTaskClass').getValue();
	
	/*
	if(subTaskClass == '' || subTaskClass == '')
		orderCond = setConds(orderCond,"result",'noPro');
	else
		orderCond = setConds(orderCond,"result",'pass');
	*/
