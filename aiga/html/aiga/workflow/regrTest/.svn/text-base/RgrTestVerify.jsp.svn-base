...
<%
	...
	String passInfo = "准发布测试评审完成，进入生产测试";
	String regnInfo = "转派其他相关人员进行此环节操作";
	String notPassInfo = "准发布测试评审不通过，发送测试人员重新执行";
	//工单Id
	...
	<body style="padding:10px 0px 60px 10px">
		<div>
			<jsp:include page="/aiga/workflow/testPlan/TestSolidTaskForm.jsp">
				...
				<jsp:param name="taskTypeName" value="自动回归测试"/>
				<jsp:param name="templateId" value="24000"/>
...
var current_linkNo = '<%=WorkflowParam.getWorkflow("rgrTestVerify").getLinkNo()%>';
//下一环节link信息
var	next_linkId ='';
var next_linkNo = '';
var	next_roleCode = '';
...
function chooseLinkInfo(){
	if(orderResult == 'pass'){
		next_linkId ='<%=WorkflowParam.getWorkflow("regrTestProdLinkTest").getLinkId()%>';
		next_linkNo = '<%=WorkflowParam.getWorkflow("regrTestProdLinkTest").getLinkNo()%>';
		next_roleCode = '<%=WorkflowParam.getWorkflow("regrTestProdLinkTest").getRoleCode()%>';
		stdMark='3';
	}else if(orderResult == 'regn'){
		next_linkId ='<%=WorkflowParam.getWorkflow("rgrTestVerify").getLinkId()%>';
		next_linkNo = '<%=WorkflowParam.getWorkflow("rgrTestVerify").getLinkNo()%>';
		next_roleCode = '<%=WorkflowParam.getWorkflow("rgrTestVerify").getRoleCode()%>';
		stdMark='1';
	}else if(orderResult == 'notPass'){
		next_linkId ='<%=WorkflowParam.getWorkflow("regrTestRunning").getLinkId()%>';
		next_linkNo = '<%=WorkflowParam.getWorkflow("regrTestRunning").getLinkNo()%>';
		next_roleCode = '<%=WorkflowParam.getWorkflow("regrTestRunning").getRoleCode()%>';
		stdMark='3';
	}
	next_templateId = '24000';
}
