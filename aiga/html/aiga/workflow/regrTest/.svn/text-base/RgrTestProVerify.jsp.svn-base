...
<%
	...
	String passInfo = "自动回归测试评审完成，代码扫描完成";
	String regnInfo = "转派其他相关人员进行此环节操作";
	String notPassInfo = "自动回归测试评审不通过，发送测试人员重新执行";
	//工单Id
...
var current_linkNo = '<%=WorkflowParam.getWorkflow("rgrTestProVerify").getLinkNo()%>';
//下一环节link信息
var	next_linkId ='';
var next_linkNo = '';
var	next_roleCode = '';
...
function chooseLinkInfo(){
	if(orderResult == 'pass'){
		...
		next_linkId ='<%=WorkflowParam.getWorkflow("rgrTestProVerify").getLinkId()%>';
		next_linkNo = '<%=WorkflowParam.getWorkflow("rgrTestProVerify").getLinkNo()%>';
		next_roleCode = '<%=WorkflowParam.getWorkflow("rgrTestProVerify").getRoleCode()%>';
		stdMark='1';
