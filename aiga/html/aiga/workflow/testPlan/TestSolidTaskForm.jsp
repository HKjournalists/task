...
Ext.onReady(function(){
	...
	var onLineTypeCombox = new Ext.form.ComboBox({
		...
		fields: ['linkId','phaseId','phaseName','vmTaskName','linkNo','vmTaskNo'],
		proxy: {
			type: 'ajax',
	...
	mapStore.load();
        ...
        var tabPanel = Ext.create("Ext.tab.Panel", {
            ...
            items: [ 
					{
					    title: "测试信息",
					    html: '<iframe id="f_0" name="f_0" scrolling="no" frameborder="0" width="100%" height="250" src="<%=request.getContextPath()%>/aiga/workflow/common/solidTypeTab.jsp?objId=${param.objId}&objType=${param.objType}&isEditor=${param.isEditor}"></iframe>'
					},
                    {
                        title: "历史轨迹",
                        ...
                        listeners:{
                        	beforeshow:function(tab,eOpts){
                        		$('#fileTrans').show();
                        		try{
                        			var _current_linkNo=current_linkNo;
                        		}catch(e){
                        			_current_linkNo=(workflowParamStore.findRecord('linkId',taskStatusCombox.getValue()).data.linkNo)+"";
                        		}
                        		finally{
                        			initPackList('${param.taskId}', '3',_current_linkNo ,'0','0',false);
                        		}
