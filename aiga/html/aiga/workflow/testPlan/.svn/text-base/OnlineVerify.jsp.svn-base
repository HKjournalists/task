...
function chooseLinkInfo(){
	if(orderResult == 'pass'){
		next_linkId ='<%=WorkflowParam.getWorkflow("onlineTrack").getLinkId()%>';
		next_linkNo = '<%=WorkflowParam.getWorkflow("onlineTrack").getLinkNo()%>';
		next_roleCode = '<%=WorkflowParam.getWorkflow("onlineTrack").getRoleCode()%>';
...
var setWOResult = function(result){
	...
	if(orderResult == 'notPass'){
		submitData();
	}else if(orderResult == 'pass' || orderResult == 'regn'){
