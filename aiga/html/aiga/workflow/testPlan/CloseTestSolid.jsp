...
Ext.onReady(function(){
		...
		items: [
			{
			    ...
			    items: [
			    	...
			    	{name:'planTag',fieldLabel:'测试计划编号'}
			    ]
			},
			{
 				xtype: 'fieldcontainer',
	...
	function closeSolidTask(){
   		var solidTaskGrid=Ext.getCmp('solidTaskGrid');
   		var closeStore=solidTaskGrid.getSelectionModel().getSelection();
   		if(closeStore.length==0){
	    	...
	    	Ext.Ajax.request({ 
				async:false,
				method:'POST',
				url:"<%=request.getContextPath()%>/closeSolidTask.do",
				params : {table :Ext.encode(data)},
				success:function(response,config){
					MaskOver();
					top.Ext.Msg.alert('提示',"任务关闭成功");
					solidTaskGrid.getStore().load();
				},
