...
public class BatchManageAction {
	...
	public void saveBatchScript(@RequestParam(value = "batchid", required = false) String batchid,
		...
		if(!StringUtils.isEmpty(scriptids)){
			...
			for(AutoBatchDetail d : delete_batchdetails){
				has_scripts.remove(d);
			}
			List<AutoBatch> batch_list = new ArrayList<AutoBatch>();
			batch_list.add(batch);
			batchService.saveOrUpdate(batch_list);
			...
			log.info("del : " + delete_batchdetails.size());
			...
			AutoBatch batch = batchService.get(batchid);
			Set<AutoBatchDetail> details = batch.getBatchdetails();
			List<AutoBatchDetail> detail_list = new ArrayList<AutoBatchDetail>();
			for(AutoBatchDetail d : details)
				detail_list.add(d);
			batch.getBatchdetails().removeAll(batch.getBatchdetails());
			List<AutoBatch> list = new ArrayList<AutoBatch>();
			list.add(batch);
			batchService.saveOrUpdate(list);
			batchDetailService.destroy(detail_list);
