Ext.define('SmartApp.controller.CaseManageCtrl', {
    ...
    stores: ['ModuleTreeStore', 'KeywordStore', 'KeywordDetailStore', 'ScriptStore', 'ScriptDetailStore', 'KeywordDetailFScriptStore'],
    ...
    openModule: function(tree, record, item, index, e){
    	...
    	if(id != null && id.substring(0, 3) == "SMD"){
    		...
    		tree.up("casemanageview").down("#keywordUpGrid").getStore().load({params:{submodule_id: id}});
    ...
    freshKeyword: function(btn){
        var store = btn.up('gridpanel').getStore();
        store.load({params:{submodule_id: btn.up("moduleview").submoduleid}});
    },
    ...
    addKeywordDetail: function(btn){
    	...
    	if( btn.up("gridpanel").keywordid == undefined){
    		Ext.Msg.alert("提示", "请先选择关键字，再增加关键字细节。");
    		return ;
    	}
    ...
    saveScript: function(btn){
    	var save_script_store = btn.up('gridpanel').getStore();
    	save_script_store.sync({
            callback : function(result){
            	if(!result.hasException)
            		Ext.Msg.alert('提示', '保存完毕');
            }
    ...
    addScriptDetail: function(btn){
    	if(btn.up('gridpanel').scriptid == undefined){
    		Ext.Msg.alert("提示", "请在上面的表格中选择具体的脚本后再增加脚本细节");
    		return;
    	}
    	var store = btn.up('gridpanel').getStore();
    ...
    showTestcase: function(btn){
    	...
    	down_grid_testcasedetail.scriptid = node.data.scriptid;
    	down_grid_testcasedetail.getStore().load({params:{testcaseid:node.data.testcaseid}});
    	if(!down_grid_testcasedetail.keworddetailScriptStore)
    		down_grid_testcasedetail.keworddetailScriptStore = Ext.create("SmartApp.store.KeywordDetailFScriptStore");
    	down_grid_testcasedetail.keworddetailScriptStore.load({params:{"scriptid":node.data.scriptid}});
    		
    	
    	
    },
    addTestcaseDetail: function(btn){
    	if(btn.up("gridpanel").testcaseid == undefined){
    		Ext.Msg.alert("提示", "请先选择上面的具体用例，再增加用例细节.");
    		return ;
    	}
    	var store = btn.up('gridpanel').getStore();
