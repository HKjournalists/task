...
public class AutoModuleService {
	...
	public void deleteModule(String id) throws AssociatiedDestroySubModuleException{
		...
		for(AutoSubModule submodule : submodules){
			submoduleService.deleteSubModule(submodule.getSubmoduleid());
		}
	...
	public void deleteModuleByName(String name){
		List<AutoModule> modules = autoModuleDao.findbyCriteria(Restrictions.eq("modulename", name));
