...
public class AutoTestcaseService {
	...
	public List<AutoTestcase> loadByScriptid(AutoScript script){
		return testcaseDao.findbyCriteria(Restrictions.eq("script", script));
	}
	
	public void destroy(List<AutoTestcase> testcases){
		List<AutoTestcaseDetail> details = testcaseDetailDao.findbyCriteria(Restrictions.in("testcase", testcases));
		for(AutoTestcaseDetail detail : details)
			testcaseDetailDao.delete(detail);
