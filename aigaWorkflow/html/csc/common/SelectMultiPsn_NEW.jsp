       ...
       <table width="100%" align="center">
			<tr>
				<td align="center">
		    		...
		    		<input name="" type="button" class="bigBtn" value="取消" id="clear"  onclick="clearData()"/>
				</td>
			</tr>
		</table>
  </body>
...
function selectThisRow(RowIndex,isSelected){
	...
	if(selectStaffIds.length-1==0&&'<%=selectStaffNames%>'!='')
		$('#selectedStakeholder').text('拟定下一环节处理人:'+'<%=selectStaffNames%>');
	if(selectStaffIds.length-1!=0)
  		$('#selectedStakeholder').text('下一环节处理人:'+condition2);
}  
function confirmData(){
	//不选人 ，直接 点击确定
	if('<%=selectStaffNames%>'==''){
		  if(selectStaffIds.length-1==0){
		  	$.messager.alert('操作提示',"请选择人员！",'error');
		  	return;
		  }
	}
	...
	if('<%=selectStaffNames%>'!=''&&totalGet==0)
		msg = '你确定选这些人吗：'+'<%=selectStaffNames%>';
	else
		msg = '你确定选这些人吗：'+condition2;
